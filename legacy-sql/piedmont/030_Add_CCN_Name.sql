...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.tmpref_ccn_by_claim_id', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.tmpref_ccn_by_claim_id;

SELECT
	[CUR_CLM_UNIQ_ID]
    ,max([PRVDR_OSCAR_NUM]) as [_CLAIM_UDF_01_]
INTO
	mi_ATM945_ACOI.dbo.tmpref_ccn_by_claim_id
FROM
	mi_ATM945_SOURCEDATA.dbo.CCLF1_CLAIMHEADER_PASSED
GROUP BY
	[CUR_CLM_UNIQ_ID]

	

IF OBJECT_ID('mi_ATM945_ACOI.dbo.tmpref_ccn', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.tmpref_ccn;
...
INTO
	mi_ATM945_ACOI.dbo.tmpref_ccn
FROM
	mi_ATM945_ACOI.dbo.tmpref_ccn_by_claim_id as b
...
FROM
	...
	left join mi_ATM945_ACOI.dbo.tmpref_ccn as b
...
(472031 row(s) affected)

(472031 row(s) affected)

(12428726 row(s) affected)
