...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_IP', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_IP;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_IP
from
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_SNF', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_SNF;
...
SELECT
	...
	,sum(paid) as paid
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_SNF
from
	(
	SELECT
		MEMBER_KEY
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_HH', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_HH;
...
SELECT
	...
	, sum(paid) as paid
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_HH
from
	(
	SELECT
		MEMBER_KEY
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r;

select
	*
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_IP
where
	typ = 'IP'
...
select top 1 @dt=to_date
	from mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r order by RowN
...
select top 1 @RowMax=max(RowN)
	from mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r


IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC0', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC0;
select top 1 *, 1 as idx
	into mi_ATM945_ACOI.dbo.A_tmp_PAC0
	from mi_ATM945_ACOI.dbo.A_tmp_PAC_IP_r order by RowN
...
BEGIN
	...
	insert into mi_ATM945_ACOI.dbo.A_tmp_PAC0
	...
	--select * from mi_ATM945_ACOI.dbo.A_tmp_PAC0
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC1', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC1;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC1
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC0 as a
	...
	left join mi_ATM945_ACOI.dbo.A_tmp_PAC_IP as d
		on a.RowN != d.RowN
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC2', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC2;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC2
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC1 as a
	left join mi_ATM945_ACOI.dbo.A_tmp_PAC_IP as b
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC3', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC3;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC3
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC2 as a
	left join mi_ATM945_ACOI.dbo.A_tmp_PAC_SNF as b
		on a.member_key = b.member_key
		and a.to_date <= b.from_date
		and DATEADD(day,30,a.to_date) > b.from_date
group by
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC4', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC4;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC4
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC3 as a
	left join mi_ATM945_ACOI.dbo.A_tmp_PAC_HH as b
		on a.member_key = b.member_key
		and a.to_date <= b.from_date
		and DATEADD(day,30,a.to_date) > b.from_date
group by
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC4 as a
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1_split', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1_split;

SELECT
	a.*
INTO
	mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1_split
FROM
	(
	SELECT
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_PAC_1', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_PAC_1;
...
SELECT
	...
	,days_wm
into
	mi_ATM945_ACOI.dbo.A_PAC_1
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_sum1_split as a
	right join (
		select
			INCURRED_YEAR_AND_MONTH
		...
		from
			mi_ATM945_ACOI.dbo.A_Claim_Summary1
		group by
			INCURRED_YEAR_AND_MONTH
			,[MEMBER_MONTH_START_DATE]
			,[ELIG_STATUS]
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG1', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG1;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG1
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_IP as a
	right join mi_ATM945_ACOI.dbo.A_Enrollment_Key as b
		on b.[MEMBER_KEY] = a.[MEMBER_KEY]
		and b.[INCURRED_YEAR_AND_MONTH] = a.[INCURRED_YEAR_AND_MONTH]
	left join ref_ACOI.dbo.ref_DRG_Family_Map as c
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG2', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG2;
...
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG2
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_IP as a
	right join mi_ATM945_ACOI.dbo.A_Enrollment_Key as b
		on b.[MEMBER_KEY] = a.[MEMBER_KEY]
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG;
...
SELECT
	...
	,a.readm_paid
into
	mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG1 as a
	left join mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG2 as b
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_PAC_2', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_PAC_2;
...
SELECT
	...
	,readm_paid
into
	mi_ATM945_ACOI.dbo.A_PAC_2
from
	mi_ATM945_ACOI.dbo.A_tmp_PAC_DRG as a
	right join (
			select
				INCURRED_YEAR_AND_MONTH
				,[MEMBER_MONTH_START_DATE]
...
(26815 row(s) affected)

(4476 row(s) affected)

(15459 row(s) affected)

(25653 row(s) affected)
...
(14389 row(s) affected)

(57556 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(58901 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(21524 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(16648 row(s) affected)

(21524 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(21246 row(s) affected)
