...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_IP_DIS', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_IP_DIS;
...
INTO
	mi_ATM945_ACOI.dbo.A_IP_DIS
FROM
	mi_ATM945_ACOI.dbo.A_Enrollment_Summary2 as c
	left join 
		(SELECT
	...
	left join
		...
		FROM
			...
			right join mi_ATM945_ACOI.dbo.A_Enrollment_Key as b
				on b.[MEMBER_KEY] = a.[MEMBER_KEY]
				and b.[INCURRED_YEAR_AND_MONTH] = a.[INCURRED_YEAR_AND_MONTH]
			left join (select distinct [PRVDR_OSCAR_NUM], [PRVDR_OSCAR_NAME] from mi_ATM945_ACOI.dbo.tmpref_ccn) as c
