...
IF OBJECT_ID('[mi_ATM945_ACOI].[dbo].[A_Enrollment_Bench_Strat]', 'U') IS NOT NULL
DROP TABLE [mi_ATM945_ACOI].[dbo].[A_Enrollment_Bench_Strat];
...
SELECT
	...
	,count(*) as MemMos
INTO
	[mi_ATM945_ACOI].[dbo].[A_Enrollment_Bench_Strat]
FROM
	(
	SELECT
		[MEMBER_MONTH_START_DATE]
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_benchCM1', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_benchCM1;
...
INTO
	mi_ATM945_ACOI.dbo.A_tmp_benchCM1
FROM
	(
	SELECT
		[MEMBER_MONTH_START_DATE]
		...
		,0 as WM_util
	FROM
		[mi_ATM945_ACOI].[dbo].[A_Enrollment_Bench_Strat] as a
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_tmp_benchCM2', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_tmp_benchCM2;
...
INTO
	mi_ATM945_ACOI.dbo.A_tmp_benchCM2
FROM
	(select * from mi_ATM945_ACOI.dbo.A_tmp_benchCM1 where flg=0) as a
	left join (select * from mi_ATM945_ACOI.dbo.A_tmp_benchCM1 where flg=1) as b
...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_Bnchmrk_Cost_Model', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_Bnchmrk_Cost_Model;
...
SELECT
	...
	,sum(WM_util) as WM_util
INTO
	mi_ATM945_ACOI.dbo.A_Bnchmrk_Cost_Model
FROM
	(select [MEMBER_MONTH_START_DATE],[INCURRED_YEAR_AND_MONTH],[ELIG_STATUS],[ASSN_STATUS],area,PCP_Name,memmos,cm_cat,LM_util,WM_util from mi_ATM945_ACOI.dbo.A_tmp_benchCM2
	UNION ALL
	select [MEMBER_MONTH_START_DATE],[INCURRED_YEAR_AND_MONTH],[ELIG_STATUS],[ASSN_STATUS],area,PCP_Name,memmos,cm_cat,LM_util,WM_util from mi_ATM945_ACOI.dbo.A_tmp_benchCM1 where flg=1) as a
GROUP BY
	a.[MEMBER_MONTH_START_DATE]
	,a.[INCURRED_YEAR_AND_MONTH]
	,a.[ELIG_STATUS]
...
(54624 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(106332 row(s) affected)

(10010 row(s) affected)

(106150 row(s) affected)
