...
IF OBJECT_ID('mi_ATM945_ACOI.dbo.A_Cost_Model_leak', 'U') IS NOT NULL
DROP TABLE mi_ATM945_ACOI.dbo.A_Cost_Model_leak;
...
SELECT
	...
	, a.paid
INTO
	mi_ATM945_ACOI.dbo.A_Cost_Model_leak
FROM
	(SELECT
		case
			when y.CCN is not null then 1
			when z.NPI is not null then 1
			else 0 end as IN_ACO_FLG
	...
	FROM
		...
		left join mi_ATM945_ACOI.dbo.A_Enrollment_Key as b
		...
		left join mi_ATM945_ACOI.dbo.tmpref_CCN_Flag as y
			on a.[PRVDR_OSCAR_NUM] = y.CCN
		left join mi_ATM945_ACOI.dbo.tmpref_NPI_Flag as z
			on a.[BILL_PROV_ID] = z.NPI
	WHERE
		b.MM = 1
