...
IF OBJECT_ID('[MI_CareNE_ACOI].[dbo].[A_Enrollment_Bench_Strat]', 'U') IS NOT NULL
DROP TABLE [MI_CareNE_ACOI].[dbo].[A_Enrollment_Bench_Strat];
...
SELECT
	...
	,count(*) as MemMos
INTO
	[MI_CareNE_ACOI].[dbo].[A_Enrollment_Bench_Strat]
FROM
	(
	SELECT
		[MEMBER_MONTH_START_DATE]
	...
	FROM
		[MI_CareNE_ACOI].[dbo].[A_Enrollment_Key]
	) as a
GROUP BY
	[MEMBER_MONTH_START_DATE]
	,[INCURRED_YEAR_AND_MONTH]
...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.A_tmp_benchCM1', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.A_tmp_benchCM1;
...
INTO
	MI_CareNE_ACOI.dbo.A_tmp_benchCM1
FROM
	(
	SELECT
		[MEMBER_MONTH_START_DATE]
		...
		,0 as WM_util
	FROM
		[MI_CareNE_ACOI].[dbo].[A_Enrollment_Bench_Strat] as a
...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.A_tmp_benchCM2', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.A_tmp_benchCM2;
...
INTO
	MI_CareNE_ACOI.dbo.A_tmp_benchCM2
FROM
	(select * from MI_CareNE_ACOI.dbo.A_tmp_benchCM1 where flg=0) as a
	left join (select * from MI_CareNE_ACOI.dbo.A_tmp_benchCM1 where flg=1) as b
...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.A_Bnchmrk_Cost_Model', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.A_Bnchmrk_Cost_Model;
...
SELECT
	...
	,sum(WM_util) as WM_util
INTO
	MI_CareNE_ACOI.dbo.A_Bnchmrk_Cost_Model
FROM
	(select [MEMBER_MONTH_START_DATE],[INCURRED_YEAR_AND_MONTH],[ELIG_STATUS],[ASSN_STATUS],area,PCP_Name,memmos,cm_cat,LM_util,WM_util from MI_CareNE_ACOI.dbo.A_tmp_benchCM2
	UNION ALL
	select [MEMBER_MONTH_START_DATE],[INCURRED_YEAR_AND_MONTH],[ELIG_STATUS],[ASSN_STATUS],area,PCP_Name,memmos,cm_cat,LM_util,WM_util from MI_CareNE_ACOI.dbo.A_tmp_benchCM1 where flg=1) as a
GROUP BY
	a.[MEMBER_MONTH_START_DATE]
	,a.[INCURRED_YEAR_AND_MONTH]
	,a.[ELIG_STATUS]
...
(25730 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(18576 row(s) affected)
