...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.dat_Claims_p', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.dat_Claims_p;
...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.tmpref_ccn_by_claim_id', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.tmpref_ccn_by_claim_id;

SELECT
	CLAIM_ID
    ,max([_CLAIM_UDF_01_]) as [_CLAIM_UDF_01_]
INTO
	MI_CareNE_ACOI.dbo.tmpref_ccn_by_claim_id
FROM
	MI_CareNE.MI.VW_CLAIM_ALLCOLS
WHERE
	[_MI_USER_DIM_08_] in ('NEXTGEN_ASSIGNED','NEXTGEN_UNASSIGNED')
GROUP BY
	CLAIM_ID

	

IF OBJECT_ID('MI_CareNE_ACOI.dbo.tmpref_ccn', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.tmpref_ccn;
...
INTO
	MI_CareNE_ACOI.dbo.tmpref_ccn
FROM
	MI_CareNE_ACOI.dbo.tmpref_ccn_by_claim_id as b
...
FROM
	MI_CareNE_ACOI.dbo.dat_Claims_p as a
	left join MI_CareNE_ACOI.dbo.tmpref_ccn as b
...
(5184683 row(s) affected)

(5184683 row(s) affected)

(9918387 row(s) affected)
