...
IF OBJECT_ID('MI_CareNE_ACOI.dbo.A_IP_DIS', 'U') IS NOT NULL
DROP TABLE MI_CareNE_ACOI.dbo.A_IP_DIS;
...
INTO
	MI_CareNE_ACOI.dbo.A_IP_DIS
FROM
	MI_CareNE_ACOI.dbo.A_Enrollment_Summary2 as c
	left join 
		(SELECT
	...
	left join
		...
		FROM
			...
			right join MI_CareNE_ACOI.dbo.A_Enrollment_Key as b
				on b.[MEMBER_KEY] = a.[MEMBER_KEY]
				and b.[INCURRED_YEAR_AND_MONTH] = a.[INCURRED_YEAR_AND_MONTH]
			left join (select distinct [PRVDR_OSCAR_NUM], [PRVDR_OSCAR_NAME] from MI_CareNE_ACOI.dbo.tmpref_ccn) as c
