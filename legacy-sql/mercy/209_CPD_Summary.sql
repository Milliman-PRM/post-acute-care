...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_CPD1', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_CPD1;
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_CPD2', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_CPD2;
...
SELECT
	...
	,b.Gender
INTO
	mi_ATM881_ACOI.dbo.A_tmp_CPD2
FROM
	(select *
	from 
		(SELECT DISTINCT [INCURRED_YEAR_AND_MONTH],[MEMBER_MONTH_START_DATE]
		FROM mi_ATM881_ACOI.dbo.A_Enrollment_Key) as x
		cross join
		(SELECT DISTINCT MEMBER_ID, ASSN_STATUS
		FROM mi_ATM881_ACOI.dbo.A_Enrollment_Key) as y) as a
	right join mi_ATM881_ACOI.dbo.A_Enrollment_Key as b
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_CPD3', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_CPD3;
...
SELECT
	...
	,sum(b.paid) as paid
INTO
	mi_ATM881_ACOI.dbo.A_tmp_CPD3
FROM
	mi_ATM881_ACOI.dbo.A_tmp_CPD2 as a
	left join mi_ATM881_ACOI.dbo.A_tmp_CPD1 as b
		on a.[MEMBER_ID] = b.[MEMBER_ID]
		and a.ASSN_STATUS = b.ASSN_STATUS
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_CPD2', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_CPD2;
...
INTO
	mi_ATM881_ACOI.dbo.A_CPD2
FROM
	mi_ATM881_ACOI.dbo.A_tmp_CPD3
GO


IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_CPD4', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_CPD4;
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_CPD5', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_CPD5;
...
SELECT
	...
	,sum(b.paid) as paid
INTO
	mi_ATM881_ACOI.dbo.A_tmp_CPD5
FROM
	mi_ATM881_ACOI.dbo.A_tmp_CPD2 as a
	left join mi_ATM881_ACOI.dbo.A_tmp_CPD4 as b
		on a.[MEMBER_ID] = b.[MEMBER_ID]
		and a.ASSN_STATUS = b.ASSN_STATUS
		and b.[MEMBER_MONTH_START_DATE] = a.[MEMBER_MONTH_START_DATE_2]
GROUP BY
...
(76707151 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(9236470 row(s) affected)

(9236470 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(23348540 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(79886687 row(s) affected)
