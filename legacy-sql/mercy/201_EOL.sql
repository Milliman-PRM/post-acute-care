...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL0a', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL0a;
...
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL0a
FROM
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL0b', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL0b;

SELECT DISTINCT
	MEMBER_KEY
	,FROM_DATE
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL0b
FROM
	mi_ATM881_ACOI.dbo.dat_claims
WHERE
	HCG_MR_LINE = 'P82b'
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL0c', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL0c;
...
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL0c
FROM
	mi_ATM881_ACOI.dbo.A_tmp_EOL0a as a
	left join mi_ATM881_ACOI.dbo.A_tmp_EOL0b as b
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL0d', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL0d;

SELECT
	MEMBER_KEY
	,sum(hospice_days) as hospice_days
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL0d
FROM
	mi_ATM881_ACOI.dbo.A_tmp_EOL0c
GROUP BY
	MEMBER_KEY
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL1', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL1;

SELECT
	a.MEMBER_KEY
	,max(b.[MEM_DOD]) as [MEM_DOD]
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL1
FROM
	mi_ATM881_ACOI.dbo.dat_MEMBMTHS as a
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL2', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL2;
...
INTO
	mi_ATM881_ACOI.dbo.A_tmp_EOL2
FROM
	mi_ATM881_ACOI.dbo.A_Enrollment_Key as a
	left join mi_ATM881_ACOI.dbo.A_tmp_EOL1 as b
	...
	left join mi_ATM881_ACOI.dbo.A_tmp_EOL0d as e
		on a.MEMBER_KEY = e.MEMBER_KEY
GO

IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_tmp_EOL3', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_tmp_EOL3;
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_EOL', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_EOL;
...
INTO
	mi_ATM881_ACOI.dbo.A_EOL
FROM
	mi_ATM881_ACOI.dbo.A_tmp_EOL2 as a
	left join mi_ATM881_ACOI.dbo.A_tmp_EOL3 as b
		on b.MEMBER_KEY = a.MEMBER_KEY
...
(274002 row(s) affected)

(274472 row(s) affected)
...
(119076 row(s) affected)

(7151874 row(s) affected)
Warning: Null value is eliminated by an aggregate or other SET operation.

(11250 row(s) affected)
