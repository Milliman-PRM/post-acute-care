...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_Enrollment_Summary4a', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_Enrollment_Summary4a;
...
SELECT
	...
	,sum(a.[AGE]) as [AGE]
INTO
	mi_ATM881_ACOI.dbo.A_Enrollment_Summary4a
FROM
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_A', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_A;
...
SELECT
	...
	,c.paid
INTO
	mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_A
FROM
	mi_ATM881_ACOI.dbo.A_Enrollment_Summary4a as d
	cross join
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Ba', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Ba;
...
SELECT
	...
	,sum(a.MM) as MM
INTO
	mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Ba
FROM
	mi_ATM881_ACOI.dbo.A_CPD2 as a
GROUP BY
	a.INCURRED_YEAR_AND_MONTH
	,a.MEMBER_MONTH_START_DATE
	,a.ELIG_STATUS
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Bb', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Bb;
...
SELECT
	...
	,sum(b.paid) as paid
INTO
	mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Bb
FROM
	mi_ATM881_ACOI.dbo.A_CPD2 as a
	left join mi_ATM881_ACOI.dbo.A_tmp_CPD5 as b
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_B', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_B;
...
INTO
	mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_B
FROM
	mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Ba as a
	cross join
	(select distinct HCG_SUBCAT1_ from ref_ACOI.dbo.ref_SvcCat_Mapping) as x
	left join mi_ATM881_ACOI.dbo.tmp_Claim_Summary2_Bb as b
...
IF OBJECT_ID('mi_ATM881_ACOI.dbo.A_Claim_Summary2', 'U') IS NOT NULL
DROP TABLE mi_ATM881_ACOI.dbo.A_Claim_Summary2;

SELECT
	a.*
INTO
	mi_ATM881_ACOI.dbo.A_Claim_Summary2
FROM
	(
	SELECT
		*
...
(47875 row(s) affected)

(1436250 row(s) affected)

(27320 row(s) affected)

(640656 row(s) affected)

(819600 row(s) affected)

(2255850 row(s) affected)
